...
<head>
    ...
    <title>All Orders - GoldSpice</title>
...
<body class="bg-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-[var(--emerald)]">All Orders</h1>
            <a href="{{ url('dashboard') }}" 
               class="bg-[var(--emerald)] text-white font-bold rounded-full px-4 py-2 hover:bg-[var(--green2)] transition transform hover:scale-105 duration-300">
            ...
            <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow">
                <thead>
                    <tr class="bg-[var(--emerald)] text-white">
                        <th class="py-3 px-1 border-b">Order ID</th>
                        <th class="py-3 px-1 border-b">User ID</th>
                        <th class="py-3 px-1 border-b">Total</th>
                        <th class="py-3 px-1 border-b">Status</th>
                        <th class="py-3 px-1 border-b">Date</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($orders as $order)
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="py-3 px-4 border-b">{{ $order->user_id }}</td>
                            <td class="py-3 px-4 border-b">Rs. {{ number_format($order->total, 2) }}</td>
                            ...
                            <td class="py-3 px-4 border-b">{{ $order->created_at->format('Y-m-d') }}</td>
                        </tr>
                    @empty
                        <tr>
